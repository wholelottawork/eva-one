...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'EVA - AI Trading System',
    ...
    binanceGuideSection1Steps:
      'Bottom nav → Futures → ⋮ (top right) → Preferences → Position Mode → select Hedge Mode  → back to Preferences → Unified Account → select Classic Trading (经典交易).',
    binanceGuideSection2: '2. Web: API Settings',
    binanceGuideSection2Steps:
      'Log in to Binance web → Profile → Settings → Account → API Management. In IP Restriction, add your server/local fixed IP. Check "Enable Futures" (允许合约). Click Save.',
    binanceGuideSection3: '3. How to Query Your Fixed IP',
    binanceGuideSection3Windows:
      'Windows: Win+R → type cmd → Enter → run: curl ifconfig.me',
    binanceGuideSection3Mac: 'Mac: Open Terminal → run: curl ifconfig.me',
    binanceGuideNote1:
      'If Binance API is blocked in your region, use a proxy and add the proxy IP to the whitelist.',
    binanceGuideNote2:
      'For self-hosted servers, enable fixed IP on the server first, then add the server IP to the whitelist.',
    closeGuide: 'Close',
    ...
    passwordRuleSpecial: 'At least one special letter',
    ...
    resetPasswordSubtitle:
      'Use email and Google Authenticator to reset your password',
    resetPasswordRedirectMsg:
    ...
    heroDescription:
      'EVA is the future standard for AI trading — an open, community-driven agentic trading OS. Supporting Binance, Aster DEX and other exchanges, self-hosted, multi-agent competition, let AI automatically make decisions, execute and optimize trades for you.',
    poweredBy: 'Powered by Aster DEX and Binance.',

    // Landing Page CTA
    readyToDefine: 'Ready to define the future of AI trading?',
    startWithCrypto:
      'Starting with crypto markets, expanding to TradFi. EVA is the infrastructure of AgentFi.',
    getStartedNow: 'Get Started Now',
    viewSourceCode: 'View Source Code',

    // Features Section
    coreFeatures: 'Core Features',
    whyChooseEva: 'Why Choose EVA?',
    openCommunityDriven:
    ...
    // About Section
    aboutEva: 'About EVA',
    whatIsEva: 'What is EVA?',
    evaNotAnotherBot:
      "EVA is not another trading bot, but the 'Linux' of AI trading —",
    evaDescription1:
    ...
    howToStart: 'How to Get Started with EVA',
    fourSimpleSteps:
    ...
    riskWarningText:
      'Dev branch is unstable, do not use funds you cannot afford to lose. EVA is non-custodial, no official strategies. Trading involves risks, invest carefully.',
    ...
    // Login Modal
    accessEvaPlatform: 'Access EVA Platform',
    loginRegisterPrompt:
    ...
    faqSubtitle: 'Find answers to common questions about EVA',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsEVA: 'What is EVA?',
    faqWhatIsEVAAnswer:
      'EVA is an open-source AI-powered trading operating system for cryptocurrency and US stock markets. It uses large language models (LLMs) like DeepSeek, GPT, Claude, Gemini to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, visual strategy builder, backtesting, and AI debate arena for consensus decisions.',

    faqHowDoesItWork: 'How does EVA work?',
    faqHowDoesItWorkAnswer:
      'EVA works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is EVA profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. EVA is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing with backtests before live trading, and understanding that past performance does not guarantee future results.',

    faqSupportedExchanges: 'Exchanges supported',
    ...
    faqUpdateEVA: 'How do I update EVA?',
    faqUpdateEVAAnswer:
      'For Docker: Run "docker compose pull && docker compose up -d" to pull latest images and restart. For manual installation: "git pull && go build -o eva" for backend, "cd web && npm install && npm run build" for frontend. Your configurations in data.db are preserved during updates.',
    ...
    faqStopLossTakeProfit: 'Does EVA support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! EVA supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqPortInUseAnswer:
      'Check what\'s using the port: "lsof -i :8080" (macOS/Linux) or "netstat -ano | findstr 8080" (Windows). Kill the process or change ports in .env: EVA_BACKEND_PORT=8081, EVA_FRONTEND_PORT=3001. Restart with "docker compose down && docker compose up -d".',

    faqFrontendNotLoading: 'Frontend shows "Loading..." forever',
    faqFrontendNotLoadingAnswer:
      'Backend may not be running or reachable. Check: 1) "curl http://127.0.0.1:8080/api/health" should return {"status":"ok"}; 2) "docker compose ps" to verify containers are running; 3) Check backend logs: "docker compose logs eva-backend"; 4) Ensure firewall allows port 8080.',

    faqDatabaseLocked: 'Database locked error',
    faqDatabaseLockedAnswer:
      'Multiple processes accessing SQLite simultaneously. Solution: 1) Stop all processes: "docker compose down" or "pkill eva"; 2) Remove lock files if present: "rm -f data/data.db-wal data/data.db-shm"; 3) Restart: "docker compose up -d". Only one backend instance should access the database.',

    faqTALibNotFound: 'TA-Lib not found during build',
    faqTALibNotFoundAnswer:
      'TA-Lib is required for technical indicators. Install: macOS: "brew install ta-lib"; Ubuntu/Debian: "sudo apt-get install libta-lib0-dev"; CentOS: "yum install ta-lib-devel". After installing, rebuild: "go build -o eva". Docker images include TA-Lib pre-installed.',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but EVA requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode"; 3) Restart your trader.',

    faqBalanceShowsZero: 'Account balance shows 0',
    faqBalanceShowsZeroAnswer:
      'Funds are likely in Spot wallet, not Futures wallet. Solution: 1) In Binance, go to Wallet → Futures → Transfer; 2) Transfer USDT from Spot to Futures; 3) Refresh EVA dashboard. Also check: funds not locked in savings/staking products.',
    ...
    faqEncryptionDetails: 'What encryption does EVA use?',
    faqEncryptionDetailsAnswer:
      'EVA uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use dedicated subaccount for EVA; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanEVAStealFunds: 'Can EVA steal my funds?',
    faqCanEVAStealFundsAnswer:
      'EVA is open-source (AGPL-3.0 license) - you can audit all code on GitHub. API keys are stored locally on YOUR machine, never sent to external servers. EVA only has the permissions you grant via API keys. For maximum safety: use API keys with trading-only permissions (no withdrawal), enable IP whitelist, use a dedicated subaccount.',
    ...
    faqCustomAIAPIAnswer:
      'Yes! EVA supports any OpenAI-compatible API. In Config → AI Models → Custom API: 1) Enter your API endpoint URL (e.g., https://your-api.com/v1); 2) Enter API key; 3) Specify model name. This works with self-hosted models, alternative providers, or Claude via third-party proxies.',

    faqAIHallucinations: 'What about AI hallucinations?',
    faqAIHallucinationsAnswer:
      'AI models can sometimes produce incorrect or fabricated information ("hallucinations"). EVA mitigates this by: 1) Providing structured prompts with real market data; 2) Enforcing JSON output format for decisions; 3) Validating orders before execution. However, AI trading is experimental - always monitor decisions and don\'t rely solely on AI judgment.',
    ...
    // ===== CONTRIBUTING =====
    faqHowToContribute: 'How can I contribute to EVA?',
    faqHowToContributeAnswer:
      'EVA is open-source and welcomes contributions! Ways to contribute: 1) Code - fix bugs, add features (check GitHub Issues); 2) Documentation - improve guides, translate; 3) Bug Reports - report issues with details; 4) Feature Ideas - suggest improvements. Start with issues labeled "good first issue". All contributors may receive airdrop rewards.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Open EVA over HTTPS (or http://localhost during development) and avoid insecure iframes/reverse proxies so the browser can enable Web Crypto.',
    ...
    twoStageKey: {
      ...
      obfuscationCopyFailed:
        'Copy failed — paste the obfuscation string manually',
    ...
    tokenomicsPage: {
      ...
      title: 'Tokenomics',
      subtitle:
        'The native utility and governance token powering the EVA protocol. Trade smarter, govern together.',
      contract: 'Contract:',
      tba: 'TBA',
      live: 'Live',
      donutSupply: '100B Supply',
      stats: {
      ...
      allocations: {
        ...
        treasury: {
          label: 'Treasury',
          desc: 'Protocol operations and long-term development',
        },
        liquidity: {
          label: 'Liquidity',
          desc: 'DEX and CEX liquidity provisioning',
        },
        ecosystem: {
          label: 'Ecosystem',
          desc: 'Integrations, partnerships, and incentive programs',
        },
      },
      ...
      vesting: {
        publicSale: { cliff: '—', vesting: '—', tge: '100%' },
        treasury: { cliff: '—', vesting: 'Ongoing', tge: '10%' },
        liquidity: { cliff: '—', vesting: 'Ongoing', tge: '100%' },
        ecosystem: { cliff: '3 months', vesting: '24 months', tge: '5%' },
      },
      ...
      utility: {
        ...
        feeDiscounts: {
          title: 'Fee Discounts',
          desc: 'Hold $EVA to unlock tiered discounts on trading fees across connected exchanges and EVA platform fees.',
        },
        staking: {
          title: 'Staking Rewards',
          desc: 'Stake $EVA to earn protocol revenue share. The longer you lock, the higher your reward multiplier.',
        },
        premiumAccess: {
    ...
    upgradePage: {
      heroTitleBefore: 'Upgrade unlocks the next layer of',
      heroTitleAfter: '',
      heroSubtitle:
      ...
      gate: {
        ...
        error: 'Unable to verify EVA balance',
        checking: 'Checking EVA balance…',
        disconnected: 'Connect a wallet to check eligibility',
        needMore: 'Need more EVA to unlock',
      },
      gateSub: {
        unconfigured:
      ...
      deepThink: {
        ...
        whitelistActive: '{count} whitelist entries active. {hint}',
        disconnected: 'Connect a wallet to activate in-wallet Deep Think.',
        prepareFailed: 'Failed to prepare wallet context.',
        chatFailed: 'Deep Think failed',
      },
      bridge: {
      ...
      whitelist: {
        ...
        labelPlaceholder: "Optional label (e.g. Mother's Ethereum wallet)",
        addressPlaceholder: 'Wallet address (0x...)',
        add: 'Add',
        saved: 'Saved',
        empty: 'No approved destinations yet.',
        footer: 'Whitelist entries are stored locally in your browser only.',
    ...
    landingPage: {
      ...
      analysisComplete:
        'Analysis complete — ask me anything about this wallet',
      followUpFailed: 'Follow-up failed.',
      connectionFailed:
        'Connection failed. Make sure the backend is running.',
      chatFailed: 'Chat failed',
      typewriter: {
        defi: 'Show me the best defi strategies.',
        movers: "What are today's top crypto movers?",
        portfolio: 'Analyze and optimize my portfolio.',
        rebalance: 'Help me rebalance my wallet.',
        staking: 'Suggest a conservative staking idea.',
        chains: 'Which chains are trending today?',
      },
      commands: {
        analyze: {
        ...
        defi: {
          label: 'Best DeFi strategies',
          prompt: 'Show me the best DeFi strategies.',
        },
        news: {
          label: 'Latest crypto news',
          prompt:
            "What's the latest crypto news? Search for the most recent headlines, market events, and notable developments in crypto.",
        },
        trend: {
          label: 'Top trending coins',
          prompt:
            'What are the top trending coins today? Search for live data.',
        },
      },
      contactsPanel: {
        title: 'Contacts',
        description: 'Save a wallet address with a name so you can type :',
        example: '"Send 1 BNB to Alice"',
        inChat: 'in chat.',
        namePlaceholder: 'Name (e.g. alice)',
        addressPlaceholder: 'Wallet address (0x...)',
        add: 'Add',
        saved: 'Saved',
        empty: 'No contacts yet. Add one above.',
        footer: 'Contacts are stored locally in your browser only.',
  ...
  zh: {
    ...
    dashboardNav: '看板',
    strategyNav: '策略工作室',
    strategyMarketNav: '策略库',
    debateNav: '辩论',
    faqNav: '帮助',

    // Footer
    footerTitle: 'EVA - AI交易系统',
    ...
    binanceGuideSection1: '1. APP 端：仓位模式与账户模式',
    binanceGuideSection1Steps:
      '底部导航栏 → 合约 → 右上角 ⋮ → 偏好设置 → 仓位模式 → 选择双向持仓 → 返回偏好设置 → 统一账户 → 选择经典交易。',
    binanceGuideSection2: '2. Web 端：API 设置',
    binanceGuideSection2Steps:
      '登录币安 web 端 → 头像 → 设置 → 账户 → API管理。在 IP 限制访问中填入服务器/本地固定 IP。勾选「允许合约」。点击保存。',
    binanceGuideSection3: '3. 如何查询自己的固定 IP',
    binanceGuideSection3Windows:
      'Windows：Win+R → 输入 cmd 回车 → 输入：curl ifconfig.me',
    binanceGuideSection3Mac: 'Mac：打开终端 → 输入：curl ifconfig.me',
    binanceGuideNote1:
      '若在国内无法使用币安 API，请使用代理服务，并将代理 IP 添加到白名单。',
    binanceGuideNote2:
      '若使用自建服务器，需先在服务器中启用固定 IP 功能，再将服务器 IP 添加到白名单。',
    closeGuide: '关闭',
    whitelistIP: '白名单IP',
    ...
    resetPasswordSubtitle: '使用邮箱和 Google 验证器 重置密码',
    resetPasswordRedirectMsg: '3秒后将自动跳转到登录页面...',
    otpGetCodeHint: '打开 Google 验证器 获取6位验证码',
    newPassword: '新密码',
    ...
    otpVerificationPrompt: '输入您验证器应用中的6位验证码',
    otpSecret: '或手动输入此密钥：',
    ...
    heroTitle2: 'Write the Trade.',
    heroDescription:
      'EVA 是 AI 交易的未来标准——一个开放、社区驱动的代理式交易操作系统。支持 Binance、Aster DEX 等交易所，自托管、多代理竞争，让 AI 为你自动决策、执行和优化交易。',
    poweredBy: '由 Aster DEX 和 Binance 提供支持。',

    // Landing Page CTA
    readyToDefine: '准备好定义 AI 交易的未来吗？',
    startWithCrypto:
      '从加密市场起步，扩展到 TradFi。EVA 是 AgentFi 的基础架构。',
    getStartedNow: '立即开始',
    viewSourceCode: '查看源码',

    // Features Section
    coreFeatures: '核心功能',
    whyChooseEva: '为什么选择 EVA？',
    ...
    aboutEva: '关于 EVA',
    whatIsEva: '什么是 EVA？',
    evaNotAnotherBot: "EVA 不是另一个交易机器人，而是 AI 交易的 'Linux' ——",
    evaDescription1: "一个透明、可信任的开源 OS，提供统一的 '决策-风险-执行'",
    evaDescription2: '层，支持所有资产类别。',
    evaDescription3:
      '从加密市场起步（24/7、高波动性完美测试场），未来扩展到股票、期货、外汇。核心：开放架构、AI',
    evaDescription4:
      '达尔文主义（多代理自竞争、策略进化）、CodeFi 飞轮（开发者 PR',
    evaDescription5: '贡献获积分奖励）。',
    ...
    riskWarningText:
      'dev 分支不稳定，勿用无法承受损失的资金。EVA 非托管，无官方策略。交易有风险，投资需谨慎。',
    ...
    // ===== 入门指南 =====
    faqWhatIsEVA: 'EVA 是什么？',
    faqWhatIsEVAAnswer:
      'EVA 是一个开源的 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统、以及用于共识决策的 AI 辩论竞技场。',

    faqHowDoesItWork: 'EVA 是如何工作的？',
    faqHowDoesItWorkAnswer:
      'EVA 分 5 步工作：1）配置 AI 模型和交易所 API 凭证；2）创建交易策略（币种选择、指标、风控）；3）创建"交易员"，组合 AI 模型 + 交易所 + 策略；4）启动交易员 - 它会定期分析市场数据并做出买入/卖出/持有决策；5）在仪表板上监控表现。AI 使用思维链（Chain of Thought）推理来解释每个决策。',

    faqIsProfitable: 'EVA 能盈利吗？',
    faqIsProfitableAnswer:
      'AI 交易是实验性的，不保证盈利。加密货币期货波动性大、风险高。EVA 仅用于教育和研究目的。我们强烈建议：从小额开始（10-50 USDT），不要投入超过承受能力的资金，在实盘交易前充分回测，并理解过去的表现不代表未来的结果。',
    ...
    faqSupportedAIModelsAnswer:
      'EVA 支持 7+ 种 AI 模型：DeepSeek（推荐性价比）、阿里云通义千问、OpenAI（GPT-5.2）、Anthropic Claude、Google Gemini、xAI Grok、Kimi（月之暗面）。您也可以使用任何 OpenAI 兼容的 API 端点。每个模型各有优势 - DeepSeek 性价比高，OpenAI 能力强但贵，Claude 擅长推理。',
    ...
    faqUpdateEVA: '如何更新 EVA？',
    faqUpdateEVAAnswer:
      'Docker 方式：运行 "docker compose pull && docker compose up -d" 拉取最新镜像并重启。手动安装：后端 "git pull && go build -o eva"，前端 "cd web && npm install && npm run build"。data.db 中的配置在更新时会保留。',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid 是去中心化交易所，需要钱包认证。步骤：1）访问 app.hyperliquid.xyz；2）连接钱包；3）生成 API 钱包（推荐）或使用主钱包；4）复制钱包地址和私钥；5）在 EVA 中添加 Hyperliquid 交易所并填入凭证。无需 KYC，完全链上。',
    ...
    faqStopLossTakeProfit: 'EVA 支持止损止盈吗？',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      '可以！EVA 支持运行 20+ 个并发交易员。每个交易员可以有不同的：AI 模型、交易所账户、策略、决策间隔。用于 A/B 测试策略、比较 AI 模型或跨交易所分散风险。在竞赛页面监控所有交易员。',
    ...
    faqPortInUseAnswer:
      '查看占用端口的进程：macOS/Linux 用 "lsof -i :8080"，Windows 用 "netstat -ano | findstr 8080"。终止进程或在 .env 中修改端口：EVA_BACKEND_PORT=8081、EVA_FRONTEND_PORT=3001。然后 "docker compose down && docker compose up -d" 重启。',

    faqFrontendNotLoading: '前端一直显示"加载中..."',
    faqFrontendNotLoadingAnswer:
      '后端可能未运行或无法访问。检查：1）"curl http://127.0.0.1:8080/api/health" 应返回 {"status":"ok"}；2）"docker compose ps" 验证容器运行中；3）查看后端日志："docker compose logs eva-backend"；4）确保防火墙允许 8080 端口。',

    faqDatabaseLocked: '数据库锁定错误',
    faqDatabaseLockedAnswer:
      '多个进程同时访问 SQLite 导致。解决方案：1）停止所有进程："docker compose down" 或 "pkill eva"；2）如有锁文件删除："rm -f data/data.db-wal data/data.db-shm"；3）重启："docker compose up -d"。只能有一个后端实例访问数据库。',

    faqTALibNotFound: '构建时找不到 TA-Lib',
    faqTALibNotFoundAnswer:
      'TA-Lib 是技术指标所需。安装：macOS："brew install ta-lib"；Ubuntu/Debian："sudo apt-get install libta-lib0-dev"；CentOS："yum install ta-lib-devel"。安装后重新构建："go build -o eva"。Docker 镜像已预装 TA-Lib。',
    ...
    faqBinancePositionModeAnswer:
      '错误："Order\'s position side does not match user\'s setting"。您处于单向持仓模式，但 EVA 需要双向持仓模式。修复：1）先平掉所有持仓；2）币安合约 → 设置（齿轮图标）→ 偏好设置 → 持仓模式 → 切换为"双向持仓"；3）重启交易员。',

    faqBalanceShowsZero: '账户余额显示 0',
    faqBalanceShowsZeroAnswer:
      '资金可能在现货钱包而非合约钱包。解决方案：1）在币安进入钱包 → 合约 → 划转；2）将 USDT 从现货划转到合约；3）刷新 EVA 仪表板。也检查：资金未被理财/质押产品锁定。',
    ...
    faqEncryptionDetails: 'EVA 使用什么加密？',
    faqEncryptionDetailsAnswer:
      'EVA 使用多层加密：1）AES-256-GCM 用于数据库存储（API 密钥、密钥）；2）RSA-2048 用于可选的传输加密（浏览器到服务器）；3）JWT 用于认证令牌。密钥在安装时生成。HTTPS 环境启用 TRANSPORT_ENCRYPTION=true。',

    faqSecurityBestPractices: '安全最佳实践是什么？',
    faqSecurityBestPracticesAnswer:
      '建议：1）使用带 IP 白名单和最小权限（仅合约交易）的交易所 API 密钥；2）为 EVA 使用专用子账户；3）远程部署启用 TRANSPORT_ENCRYPTION；4）切勿分享 .env 或 data.db 文件；5）使用有效证书的 HTTPS；6）定期轮换 API 密钥；7）监控账户活动。',

    faqCanEVAStealFunds: 'EVA 会盗取我的资金吗？',
    faqCanEVAStealFundsAnswer:
      'EVA 是开源的（AGPL-3.0 许可）- 您可以在 GitHub 审计所有代码。API 密钥存储在您的机器本地，从不发送到外部服务器。EVA 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      '可以！EVA 支持任何 OpenAI 兼容的 API。在配置 → AI 模型 → 自定义 API 中：1）输入 API 端点 URL（如 https://your-api.com/v1）；2）输入 API 密钥；3）指定模型名称。适用于自托管模型、替代提供商或通过第三方代理的 Claude。',

    faqAIHallucinations: 'AI 幻觉问题怎么办？',
    faqAIHallucinationsAnswer:
      'AI 模型有时会产生不正确或虚构的信息（"幻觉"）。EVA 通过以下方式缓解：1）提供带真实市场数据的结构化提示词；2）强制 JSON 输出格式；3）执行前验证订单。但 AI 交易是实验性的 - 始终监控决策，不要完全依赖 AI 判断。',
    ...
    // ===== 参与贡献 =====
    faqHowToContribute: '如何为 EVA 做贡献？',
    faqHowToContributeAnswer:
      'EVA 是开源项目，欢迎贡献！贡献方式：1）代码 - 修复 bug、添加功能（查看 GitHub Issues）；2）文档 - 改进指南、翻译；3）Bug 报告 - 详细报告问题；4）功能建议 - 提出改进意见。从标记为"good first issue"的问题开始。所有贡献者可能获得空投奖励。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        '请通过 HTTPS 或本机 localhost 访问 EVA，并避免嵌入不安全 iframe/反向代理，以符合浏览器的 Web Crypto 规则。',
    ...
    tokenomicsPage: {
      ...
      title: '代币经济',
      subtitle:
        'EVA 协议的原生实用与治理代币。更聪明地交易，共同治理。',
      contract: '合约：',
      tba: '待定',
      live: '已上线',
      donutSupply: '1000 亿总量',
      stats: {
        totalSupply: '总供应量',
        tokens: '枚代币',
        launchPlatform: '发射平台',
      ...
      vesting: {
        publicSale: { cliff: '—', vesting: '—', tge: '100%' },
        treasury: { cliff: '—', vesting: '持续释放', tge: '10%' },
        liquidity: { cliff: '—', vesting: '持续释放', tge: '100%' },
        ecosystem: { cliff: '3 个月', vesting: '24 个月', tge: '5%' },
      },
      utilityTitle: '代币效用',
      utility: {
        governance: {
          title: '治理',
          desc: '对协议升级、费率结构与生态赠款分配进行投票。一币一票。',
        },
        feeDiscounts: {
          title: '费率折扣',
          desc: '持有 $EVA 可在接入交易所与 EVA 平台费用上解锁分级折扣。',
        },
        staking: {
          title: '质押奖励',
          desc: '质押 $EVA 分享协议收益。锁仓越久，奖励倍数越高。',
        },
        premiumAccess: {
          title: '高级权限',
          desc: '持币者解锁高级 AI 模型、更高策略上限与优先交易执行。',
        },
      },
      cta: {
        titleBefore: '尽早加入',
        titleAfter: '',
        subtitle: '关注我们的公告，获取发射详情、白名单机会与空投活动。',
        followX: '关注 X',
        github: 'GitHub',
        learnMore: '了解更多',
      },
    ...
    upgradePage: {
      heroTitleBefore: 'Upgrade 解锁',
      heroTitleAfter: '的下一层能力',
      heroSubtitle:
      ...
      gate: {
        ...
        error: '无法验证 EVA 余额',
        checking: '正在检查 EVA 余额…',
        disconnected: '连接钱包以检查资格',
        needMore: '需要更多 EVA 才能解锁',
      },
      gateSub: {
        unconfigured:
      ...
      deepThink: {
        ...
        subtitle: '深度思考模式',
        description:
          '面向高上下文钱包任务、路径规划与自然语言执行请求的门槛助手层。',
        locked: '持有 150,000 EVA 解锁',
        prepare: '准备钱包上下文',
        preparing: '正在准备钱包上下文…',
        placeholder:
          '请 EVA 桥接、兑换、再平衡，或向白名单名称转账…',
        send: '发送',
        active: '已激活',
        bridge: '桥接工具',
        whitelist: '管理白名单',
        bridgeHint: '在聊天中生成上下文，再用下方内置桥接工具执行跨链路径。',
        ready:
          '钱包上下文已就绪。Deep Think 现已使用您连接的钱包与已批准名称解析。',
        whitelistEmpty:
          '尚无白名单条目。未批准的名称将按纯文本发送。',
        whitelistActive: '已有 {count} 条白名单。{hint}',
        disconnected: '连接钱包以激活钱包内 Deep Think。',
        prepareFailed: '准备钱包上下文失败。',
        chatFailed: 'Deep Think 失败',
      },
      bridge: {
        title: '跨链桥接',
        subtitle: '跨链桥接',
        kicker: '合格持有者的最优路径桥接执行',
        blurb:
          'EVA 将在支持的链上预计算最优流动性路径，无需手动路由即可完成桥接与兑换。',
        from: '来源链',
        to: '目标链',
        payToken: '支付代币',
        receiveToken: '接收代币',
        ...
        route: '加载路径',
        locked: '持有 150,000 EVA 解锁',
        connectHint: '请使用页面上方的连接钱包按钮以激活桥接执行。',
        note: '此面板复用应用中已有的 Squid 实时执行路径。',
      },
      whitelist: {
        title: '转账白名单',
        description:
          '仅保存在此处的名称会在 Upgrade 操作中被解析，从而将自然语言转账限制在已批准的目标地址。',
        namePlaceholder: '名称（例如 mom）',
        labelPlaceholder: '可选标签（例如：母亲的以太坊钱包）',
        addressPlaceholder: '钱包地址 (0x...)',
        add: '添加',
        saved: '已保存',
        empty: '尚无已批准的目标地址。',
        footer: '白名单条目仅保存在您的浏览器本地。',
        deleteTitle: '删除白名单条目',
        errors: {
          nameRequired: '请填写名称。',
          addressRequired: '请填写地址。',
          invalidAddress: '地址无效。必须是有效的 EVM (0x...) 或 Solana 地址。',
        },
      },
    },
    ...
    landingPage: {
      ...
      goodResponse: '回答有帮助',
      badResponse: '回答无帮助',
      analysisComplete: '分析完成 — 你可以问我更多关于这个钱包的问题',
      followUpFailed: '后续请求失败。',
      connectionFailed: '连接失败。请确认后端服务正在运行。',
      chatFailed: '对话失败',
      typewriter: {
        defi: '展示最佳 DeFi 策略。',
        movers: '今天涨幅最大的加密货币有哪些？',
        portfolio: '分析并优化我的投资组合。',
        rebalance: '帮我再平衡钱包。',
        staking: '推荐一个保守的质押方案。',
        chains: '今天哪些链最热门？',
      },
      commands: {
        ...
        news: {
          label: '最新 crypto 新闻',
          prompt:
            '最新的加密货币新闻有哪些？搜索最近头条、市场事件与重要动态。',
        },
        trend: {
          label: '热门趋势币种',
          prompt: '今天最热门的趋势币种有哪些？搜索实时数据。',
        },
      },
      contactsPanel: {
        title: '联系人',
        description: '为钱包地址设置名称，这样在聊天中可以输入：',
        example: '「向 Alice 发送 1 BNB」',
        inChat: '。',
        namePlaceholder: '名称（例如 alice）',
        addressPlaceholder: '钱包地址 (0x...)',
        add: '添加',
        saved: '已保存',
        empty: '尚无联系人。请在上方添加。',
        footer: '联系人仅保存在您的浏览器本地。',
        deleteTitle: '删除联系人',
        errors: {
          nameRequired: '请填写名称。',
          addressRequired: '请填写地址。',
